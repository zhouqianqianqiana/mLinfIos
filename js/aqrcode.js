/**
 * 示例：
qrcode(getDom("#qrcodeCanvas"), // 二维码嵌入的dom节点
{
...
for (function(a) {
	//if(jQuery && a == jQuery) a = a.fn;
    a.qrcode = function(dom, b) {
        ...
        d = function() {
			...
			css(d, "width :" + b.width + "px;" 
				+ "height:" + b.height + "px;"
				+ "background-color:" + b.background + ";"
				+ "border:0px;"
				+ "border-collapse:collapse;")
            //这里的utf16to8(b.text)是对Text中的字符串进行转码，让其支持中文
            for (c.addData(utf16to8(b.text)), c.make(), e = b.width / c.getModuleCount(), f = b.height / c.getModuleCount(), g = 0; g < c.getModuleCount(); g++) {
                h = document.createElement("tr");
				append(d, css(h, "height: " + f + "px;"))
				for (i = 0; i < c.getModuleCount(); i++) {
					var t = document.createElement("td");
					css(t, "background-color:" + (c.isDark(g, i) ? b.foreground: b.background) + ";");
					append(h, t);
                }
            }
            return d
